...
1. `main` から通し番号の作業 PR を作る（次の番号を引き継ぐ）
2. ローカルで `npm ci`、DB が必要なら `npm run setup`
3. 変更後に `npm run verify`
4. PR の CI が成功してから `main` へマージ
5. GitHub 連携済みの Vercel が自動デプロイし、コミットの `Vercel` ステータスが成功したことを確認

DB スキーマ変更を含む場合は、通常の検証に加えて空の PostgreSQL に対する
`prisma migrate deploy` を確認してからマージします。
